...
void check_mouse(XEvent *e)
{
		...
		if (e->xbutton.button==1) {
			...
			if (ts > 0.1) {
				...
				if (g.nbullets < MAX_BULLETS) {
					...
					b->pos[0] += hori*20.0f;
					b->pos[1] += vert*20.0f;
					b->vel[0] += hori*6.0f + rnd()*0.1;
					b->vel[1] += vert*6.0f + rnd()*0.1;
					b->color[0] = 1.0f;
					b->color[1] = 1.0f;
					b->color[2] = 1.0f;
					++g.nbullets;
	...
	if (e->type == MotionNotify) {
		if (savex != e->xbutton.x || savey != e->xbutton.y) {
			//Mouse moved
			int hmove = savex - e->xbutton.x;
			int vmove = savey - e->xbutton.y;
			if (++ct < 10)
				return;		
			if (hmove > 0) {
				//mouse moved along the x-axis.
				g.ship.angle += 0.05f * (float)hmove;
			...
			if (vmove > 0) {
				...
				Flt hori = cos(rad);
				Flt vert = sin(rad);
				g.ship.vel[0] += hori * (float)vmove * 0.001f;
				g.ship.vel[1] += vert * (float)vmove * 0.001f;
